...
import com.sjhy.platform.biz.deploy.cache.CacheProvider;
import com.sjhy.platform.biz.deploy.cache.ehcache.EhCacheProvider;
...
public class RedisCacheManager {
	 ...
	 */
	public static void initCacheProvider(RedisCacheChannel listener){

		
		RedisCacheManager.listener = listener;
		try{

			Properties props = KrCache.getConfig();
			
			RedisCacheManager.l1_provider = getProviderInstance(props.getProperty("cache.L1.provider_class"));
			RedisCacheManager.l1_provider.start(getProviderProperties(props, RedisCacheManager.l1_provider));
			log.info("Using L1 CacheProvider : " + l1_provider.getClass().getName());
			
			RedisCacheManager.l2_provider = getProviderInstance(props.getProperty("cache.L2.provider_class"));
			RedisCacheManager.l2_provider.start(getProviderProperties(props, RedisCacheManager.l2_provider));
			log.info("Using L2 CacheProvider : " + l2_provider.getClass().getName());
			
			RedisCacheManager.serializer = props.getProperty("cache.serialization");
	...
	private final static ICache _GetCache(int level, String cache_name, boolean autoCreate) {
	...
	public final static Object get(int level, String name, Object key){
		...
		if(name!=null && key != null) {
			ICache cache = _GetCache(level, name, false);
            if (cache != null)
	...
	public final static void set(int level, String name, Object key, Object value){
		...
		if(name!=null && key != null && value!=null) {
			ICache cache =_GetCache(level, name, true);
            if (cache != null)
                cache.put(key,value);
        }
	}
