...
import com.sjhy.platform.biz.deploy.cache.CacheExpiredListener;
 ...
 *
 */
public class RedisCacheChannel extends BinaryJedisPubSub implements CacheExpiredListener, ICacheChannel {

    private final static Logger log = LoggerFactory.getLogger(RedisCacheChannel.class);

    private String name;
    private static String channel = "com/kairo";
    ...
    private RedisCacheChannel(String name) throws CacheException {
        ...
        try {
            ...
            RedisCacheManager.initCacheProvider(this);
    ...
    public CacheObject get(String region, Object key) {
        ...
        if (region != null && key != null) {
            obj.setValue(RedisCacheManager.get(LEVEL_1, region, key));
            if (obj.getValue() == null) {
                obj.setValue(RedisCacheManager.get(LEVEL_2, region, key));
                if (obj.getValue() != null) {
                    obj.setLevel(LEVEL_2);
                    RedisCacheManager.set(LEVEL_1, region, key, obj.getValue());
                }
            } else
     ...
     * @return cache object
     */
    public CacheObject getFromRedis(String region, Object key) {
        CacheObject obj = new CacheObject();
        obj.setRegion(region);
        obj.setKey(key);
    ...
    public void set(String region, Object key, Object value) {
        if (region != null && key != null) {
            if (value == null)
                ...
                RedisCacheManager.set(LEVEL_1, region, key, value);
                RedisCacheManager.set(LEVEL_2, region, key, value);
                _sendEvictCmd(region, key);// 清除原有的一级缓存的内容
            }
        }
    ...
    public void set(String region, Object key, Object value, Integer expireInSec) {
        if (region != null && key != null) {
            if (value == null)
                ...
                // 4. L1 和 L2 都有
                // _sendEvictCmd(region, key);// 清除原有的一级缓存的内容
                RedisCacheManager.set(LEVEL_1, region, key, value, expireInSec);
                RedisCacheManager.set(LEVEL_2, region, key, value, expireInSec);
                _sendEvictCmd(region, key);// 清除原有的一级缓存的内容
            }
        }
        // log.info("write data to cache region="+region+",key="+key+",value="+value);
    ...
    public void evict(String region, Object key) {
        RedisCacheManager.evict(LEVEL_1, region, key); // 删除一级缓存
        RedisCacheManager.evict(LEVEL_2, region, key); // 删除二级缓存
    ...
    public void batchEvict(String region, List keys) {
        RedisCacheManager.batchEvict(LEVEL_1, region, keys);
        RedisCacheManager.batchEvict(LEVEL_2, region, keys);
    ...
    public void clear(String region) throws CacheException {
        RedisCacheManager.clear(LEVEL_1, region);
        RedisCacheManager.clear(LEVEL_2, region);
    ...
    public List keys(String region) throws CacheException {
        return RedisCacheManager.keys(LEVEL_1, region);
    }
    ...
    public void notifyElementExpired(String region, Object key) {
        ...
        if (key instanceof List)
            RedisCacheManager.batchEvict(LEVEL_2, region, (List) key);
        else
            RedisCacheManager.evict(LEVEL_2, region, key);
    ...
    protected void onDeleteCacheKey(String region, Object key) {
        if (key instanceof List)
            RedisCacheManager.batchEvict(LEVEL_1, region, (List) key);
        else
            RedisCacheManager.evict(LEVEL_1, region, key);
     ...
     */
    public void close() {
        RedisCacheManager.shutdown(LEVEL_1);
        
        if (isSubscribed()) {
            this.unsubscribe();
        }
        
        RedisCacheManager.shutdown(LEVEL_2);
    }
