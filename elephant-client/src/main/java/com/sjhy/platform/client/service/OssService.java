...
public interface OssService {
    ...
    // 获取oss文件Bucketkeys
    ResultDTO<List> getBucketkeys(ServiceContext sc, int keyType);

    // 取得用户上传信息
    ResultDTO<List> getBucket(ServiceContext sc);

    // 更新上传进度(客户端通知服务器上传结束)
    ResultDTO notifyEndPut(ServiceContext sc, int operateId, int result, String md5);

    // 申请上传操作
    ResultDTO<AliOssBucketVO> putBucket(ServiceContext sc, String objKey, long gold, String saveTime, String fname, int objType);
}
