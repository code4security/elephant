...
public class XmlUtils {
	...
	public static Map<String, String> doParseXml(String xml) throws Exception {
		...
		for (int i = 0; i < max; i++) {
			...
			for (int j = 0; j < n.getChildNodes().getLength(); j++) {
				Node _n = n.getChildNodes().item(j);
				if (_n.getNodeType() == Node.TEXT_NODE || _n.getNodeType() == Node.CDATA_SECTION_NODE) {
					m.put(n.getNodeName(), _n.getNodeValue());
				}
			}
		}
	 ...
	 * @return
	 */
	public static String map2xmlBody(Map<String, String> packageParams){
